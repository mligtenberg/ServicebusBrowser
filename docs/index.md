...
- [Virtual Scroll Geometry (CDK)](./virtual-scroll-geometry.md): The two invariants the CDK fixed-size strategy silently depends on — a viewport size that is re-measured on element-level resizes, and a row whose outer height equals `itemSize` — and why breaking them makes the last rows of a huge list unreachable — plus the 100k-row pagination threshold above which one viewport stops being usable, and the chunk-relative/absolute index contract it introduces.
