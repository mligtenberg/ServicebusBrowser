...
export class TopologyQueueEffects {
  ...
  loadQueues$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ namespaceId }) =>
        from(this.serviceBusClient.listQueues(namespaceId)).pipe(
          catchError((error) => [
            {
              problem: {
          ...
          map(({ queues, namespace }) => {
            if (queues instanceof Array) {
              return internalActions.queuesLoaded({
                namespace: namespace!,
                queues,
              });
            }
            return internalActions.failedToLoadQueues({
              namespace: namespace!,
              error: queues.problem,
            });
          })
        )
      )
  ...
  loadQueue$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ namespaceId, queueId }) =>
        from(this.serviceBusClient.getQueue(namespaceId, queueId)).pipe(
          catchError((error) => [
            {
              problem: {
          ...
          map(({ queue, namespace }) => {
            if ('problem' in queue) {
              return internalActions.failedToLoadQueue({
                namespace: namespace!,
                error: queue.problem,
              });
            }
            return internalActions.queueLoaded({
              namespace: namespace!,
              queue: queue,
            });
          })
  ...
  addQueue$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ namespaceId, queue }) =>
        from(this.serviceBusClient.createQueue(namespaceId, queue)).pipe(
          map(() => queue),
          catchError((error) => [
            {
              problem: {
          ...
          map(({ namespace, queue }) => {
            if ('problem' in queue) {
              return internalActions.failedToAddQueue({
                namespace: namespace!,
                error: queue.problem,
              });
            }
            return internalActions.queueAdded({
              namespace: namespace!,
              queue: queue,
            });
          })
  ...
  editQueue$ = createEffect(() =>
    this.actions$.pipe(
      ofType(actions.editQueue),
      mergeMap(({ namespaceId, queue }) =>
        from(this.serviceBusClient.editQueue(namespaceId, queue)).pipe(
          map(() => queue),
          catchError((error) => [
            {
              problem: {
                title: 'failed to edit queue',
                detail: error.toString(),
              },
            },
          ]),
          ...
          map(({ namespace, queue }) => {
            if ('problem' in queue) {
              return internalActions.failedToEditQueue({
                namespace: namespace!,
                error: queue.problem,
              });
            }
            return internalActions.queueEdited({
              namespace: namespace!,
              queue: queue,
            });
          })
  ...
  removeQueue$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ namespaceId, queueId }) =>
        from(this.serviceBusClient.removeQueue(namespaceId, queueId)).pipe(
          map(() => queueId),
          catchError((error) => [
            {
              problem: {
                title: 'failed to remove queue',
                detail: error.toString(),
              },
            },
          ]),
          ...
          map(({ namespace, queueId }) => {
            if (typeof queueId === 'object') {
              return internalActions.failedToEditQueue({
                namespace: namespace!,
                error: queueId.problem,
              });
            }
            return internalActions.queueRemoved({
              namespace: namespace!,
              queueId: queueId,
            });
          })
  ...
  loadQueuesOnNamespaceLoaded$ = createEffect(() =>
    this.actions$.pipe(
  ...
  loadQueueOnQueueAdded$ = createEffect(() =>
    this.actions$.pipe(
      ofType(internalActions.queueAdded),
      mergeMap(({ queue, namespace }) =>
        from([
          actions.loadQueue({ namespaceId: namespace.id, queueId: queue.id }),
        ])
      )
    )
  );

  reloadQueueOnClearQueue$ = createEffect(() =>
    this.actions$.pipe(
