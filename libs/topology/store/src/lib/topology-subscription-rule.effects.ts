...
export class TopologySubscriptionRuleEffects {
  ...
  addSubscriptionRule$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ namespaceId, topicId, subscriptionId, rule }) =>
        from(
          this.serviceBusClient.createSubscriptionRule(
            namespaceId,
            topicId,
            subscriptionId,
            rule
          ...
          catchError((error) => [
            {
              problem: {
                title: 'failed to add subscription rule',
          ...
          map(({ namespace, topic, subscription, rule: updatedRule }) => {
            if ('problem' in updatedRule) {
              return internalActions.failedToAddSubscriptionRule({
                namespace,
                topic,
                subscription,
                ruleName: rule.name,
                error: updatedRule.problem,
              });
            }

            return internalActions.subscriptionRuleAdded({
              namespace,
              topic,
              subscription,
              rule: updatedRule,
  ...
  editSubscriptionRule$ = createEffect(() =>
    this.actions$.pipe(
      ofType(actions.editSubscriptionRule),
      mergeMap(({ namespaceId, topicId, subscriptionId, rule }) =>
        from(
          this.serviceBusClient.editSubscriptionRule(
            namespaceId,
            topicId,
            subscriptionId,
            rule
          ...
          catchError((error) => [
            {
              problem: {
                title: 'failed to edit subscription rule',
                detail: error.toString(),
              },
            },
          ]),
          ...
          map(({ namespace, topic, subscription, rule: updatedRule }) => {
            if ('problem' in updatedRule) {
              return internalActions.failedToEditSubscriptionRule({
                namespace,
                topic,
                subscription,
                ruleName: rule.name,
            ...
            return internalActions.subscriptionRuleEdited({
              namespace,
              topic,
              subscription,
              rule: updatedRule,
  ...
  deleteSubscriptionRule$ = createEffect(() =>
    this.actions$.pipe(
      ofType(actions.removeSubscriptionRule),
      mergeMap(({ namespaceId, topicId, subscriptionId, ruleName }) =>
        from(
          this.serviceBusClient.removeSubscriptionRule(
            namespaceId,
            topicId,
            subscriptionId,
            ruleName
          ...
          catchError((error) => [
            {
              problem: {
                title: 'failed to delete subscription rule',
                detail: error.toString(),
              },
            },
          ]),
          ...
          map(
            ({
              ...
              if (originalRuleName instanceof Object) {
                return internalActions.failedToRemoveSubscriptionRule({
                  namespace,
                  topic,
                  subscription,
                  ruleName: ruleName,
                  error: originalRuleName.problem,
                });
              }

              return internalActions.subscriptionRuleRemoved({
                namespace,
                topic,
                subscription,
                ruleName,
