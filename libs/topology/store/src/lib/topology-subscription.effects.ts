...
export class TopologySubscriptionEffects {
        ...
        from(
          ...
          map(({ subscriptions, namespace, topic }) => {
            if (subscriptions instanceof Array) {
              return internalActions.subscriptionsLoaded({
                namespace: namespace!,
                topic: topic!,
                subscriptions,
              });
            }
            return internalActions.failedToLoadSubscriptions({
              namespace: namespace!,
              topic: topic!,
  ...
  loadSubscription$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ namespaceId, topicId, subscriptionId }) =>
        from(
          ...
          map(({ subscription, namespace, topic, subscriptionId }) => {
            if ('problem' in subscription) {
              return internalActions.failedToLoadSubscription({
                namespace: namespace!,
                topic: topic!,
                subscriptionId,
                error: subscription.problem,
              });
            }
            return internalActions.subscriptionLoaded({
              namespace: namespace!,
              topic: topic!,
  ...
  addSubscription$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ namespaceId, topicId, subscription }) =>
        from(
          ...
          map(({ namespace, subscription, topic }) => {
            if ('problem' in subscription) {
              return internalActions.failedToAddSubscription({
                namespace: namespace!,
                topic: topic!,
                error: subscription.problem,
              });
            }
            return internalActions.subscriptionAdded({
              namespace: namespace!,
              topic: topic!,
              subscription: subscription,
            });
      ...
      mergeMap(({ namespaceId, topicId, subscription }) =>
        from(
          ...
          catchError((error) => [
            {
              problem: {
                title: 'failed to edit subscription',
                detail: error.toString(),
              },
            },
          ]),
          ...
          map(({ namespace, subscription, topic }) => {
            if ('problem' in subscription) {
              return internalActions.failedToEditSubscription({
                namespace: namespace!,
                topic: topic!,
                error: subscription.problem,
              });
            }
            return internalActions.subscriptionEdited({
              namespace: namespace!,
              topic: topic!,
              subscription: subscription,
            });
        ...
        from(
          ...
          map(({ namespace, subscriptionId, topic }) => {
            if (typeof subscriptionId === 'object') {
              return internalActions.failedToRemoveSubscription({
                namespace: namespace!,
                topic: topic!,
                error: subscriptionId.problem,
              });
            }
            return internalActions.subscriptionRemoved({
              namespace: namespace!,
              topic: topic!,
