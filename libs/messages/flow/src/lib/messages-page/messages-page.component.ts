...
export class MessagesPageComponent {
  ...
  filteredMessages = toSignal(
    toObservable(this.messageFilter).pipe(
      switchMap((filter) =>
        from(repository.getMessages(this.currentPage()!.id, filter)),
      ),
    ),
    {
      initialValue: [],
  ...
  getMenuItems(
    menuSelection: unknown | unknown[] | undefined,
    allMessages: boolean,
  ) {
    if (!menuSelection) {
      return [];
