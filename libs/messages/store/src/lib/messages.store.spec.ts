...
describe('messages store reducer', () => {
  ...
  it('marks page.ts loaded and sets name range', () => {
    ...
    state = logsReducer(
      state,
      Actions.peekMessagesLoadingDone({ pageId, endpoint })
    );
  ...
  it('adds page.ts for imported messages', () => {
    ...
    const state = logsReducer(
      initialState,
      InternalActions.messagesImported({ pageName: 'file', messages: msgs })
    );
    expect(state.receivedMessages.length).toBe(1);
  ...
  it('sets batch resend messages', () => {
    ...
    const state = logsReducer(
      initialState,
      Actions.setBatchResendMessages({ messages: msgs })
    );
