...
provideActionHandler('service-bus:topic:add', async (action) => {
...
provideActionHandler('service-bus:topic:edit', async (action) => {
  const router = inject(Router);
  await router.navigate([
    'manage-service-bus',
    'connections',
...
provideActionHandler('service-bus:topic:delete', async (action) => {
  ...
  if (
    ...
    throw new Error('Connection ID and topic name are required parameters');
  }
  ...
  if (result) {
    await managementClient.removeTopic(connectionId, topicName);
  }
