...
export class AdministrationClient {
  ...
  async getQueues(): Promise<QueueWithMetaData[]> {
    ...
    for await (const queue of queuesPages) {
      const runtimeProps = await administrationClient.getQueueRuntimeProperties(
        queue.name,
      );
  ...
  async addQueue(queue: QueueWithMetaData): Promise<void> {
    const administrationClient = this.getAdministrationClient();
    const body = {
      autoDeleteOnIdle: !queue.properties.autoDeleteOnIdle
        ? undefined
        : queue.properties.autoDeleteOnIdle,
      defaultMessageTimeToLive: queue.properties.defaultMessageTimeToLive,
      duplicateDetectionHistoryTimeWindow: !queue.properties
        .duplicateDetectionHistoryTimeWindow
        ? undefined
        : queue.properties.duplicateDetectionHistoryTimeWindow,
      forwardDeadLetteredMessagesTo:
        queue.properties.forwardDeadLetteredMessagesTo ?? undefined,
      ...
      userMetadata: !queue.properties.userMetadata
        ? undefined
        : queue.properties.userMetadata,
      ...
      deadLetteringOnMessageExpiration:
        queue.settings.deadLetteringOnMessageExpiration,
  ...
  async updateQueue(queue: QueueWithMetaData): Promise<void> {
    ...
    queueProperties.autoDeleteOnIdle = !queue.properties.autoDeleteOnIdle
      ? queueProperties.autoDeleteOnIdle
      : queue.properties.autoDeleteOnIdle;
    queueProperties.defaultMessageTimeToLive =
      queue.properties.defaultMessageTimeToLive;
    queueProperties.duplicateDetectionHistoryTimeWindow = !queue.properties
      .duplicateDetectionHistoryTimeWindow
      ? queueProperties.duplicateDetectionHistoryTimeWindow
      : queue.properties.duplicateDetectionHistoryTimeWindow;
    queueProperties.forwardDeadLetteredMessagesTo =
      queue.properties.forwardDeadLetteredMessagesTo ?? undefined;
    queueProperties.forwardTo = queue.properties.forwardMessagesTo ?? undefined;
    queueProperties.lockDuration = !queue.properties.lockDuration
      ? queueProperties.lockDuration
      : queue.properties.lockDuration;
    queueProperties.userMetadata = !queue.properties.userMetadata
      ? ''
      : queue.properties.userMetadata;
    queueProperties.enableBatchedOperations =
      queue.settings.enableBatchedOperations;
  ...
  async getTopics(): Promise<TopicWithMetaData[]> {
    ...
    for await (const topic of topicsPages) {
      const topicMeta = await administrationClient.getTopicRuntimeProperties(
        topic.name,
      );
  ...
  async addTopic(topic: Topic): Promise<void> {
    const administrationClient = this.getAdministrationClient();
    const body = {
      autoDeleteOnIdle: !topic.properties.autoDeleteOnIdle
        ? undefined
        : topic.properties.autoDeleteOnIdle,
      defaultMessageTimeToLive: topic.properties.defaultMessageTimeToLive,
      duplicateDetectionHistoryTimeWindow: !topic.properties
        .duplicateDetectionHistoryTimeWindow
        ? undefined
        : topic.properties.duplicateDetectionHistoryTimeWindow,
      maxSizeInMegabytes: topic.properties.maxSizeInMegabytes,
      userMetadata: !topic.properties.userMetadata
        ? undefined
        : topic.properties.userMetadata,
  ...
  async updateTopic(topic: Topic): Promise<void> {
    ...
    topicProperties.autoDeleteOnIdle = !topic.properties.autoDeleteOnIdle
      ? topicProperties.autoDeleteOnIdle
      : topic.properties.autoDeleteOnIdle;
    topicProperties.defaultMessageTimeToLive =
      topic.properties.defaultMessageTimeToLive;
    topicProperties.duplicateDetectionHistoryTimeWindow = !topic.properties
      .duplicateDetectionHistoryTimeWindow
      ? topicProperties.duplicateDetectionHistoryTimeWindow
      : topic.properties.duplicateDetectionHistoryTimeWindow;
    topicProperties.maxSizeInMegabytes = topic.properties.maxSizeInMegabytes;
    topicProperties.userMetadata = !topic.properties.userMetadata
      ? ''
      : topic.properties.userMetadata;
    topicProperties.enableBatchedOperations =
      topic.settings.enableBatchedOperations;
  ...
  async getSubscriptions(topicId: string): Promise<SubscriptionWithMetaData[]> {
    ...
    for await (const subscription of subscriptionsPages) {
      const [subscriptionMeta, rules] = await Promise.all([
        administrationClient.getSubscriptionRuntimeProperties(
          topicId,
          subscription.subscriptionName,
        ),
        this.getSubscriptionRules(
      ...
      subscriptions.push(
        this.mapSubscription(topicId, subscription, subscriptionMeta, rules),
      );
    }
  ...
  async addSubscription(
    ...
    const body: CreateSubscriptionOptions = {
      autoDeleteOnIdle: !subscription.properties.autoDeleteOnIdle
        ? undefined
        : subscription.properties.autoDeleteOnIdle,
      defaultMessageTimeToLive:
        subscription.properties.defaultMessageTimeToLive,
      forwardDeadLetteredMessagesTo:
        subscription.properties.forwardDeadLetteredMessagesTo ?? undefined,
      forwardTo: subscription.properties.forwardMessagesTo ?? undefined,
      lockDuration: !subscription.properties.lockDuration
        ? undefined
        : subscription.properties.lockDuration,
      userMetadata: !subscription.properties.userMetadata
        ? undefined
        : subscription.properties.userMetadata,
      ...
      deadLetteringOnFilterEvaluationExceptions:
        subscription.settings.deadLetteringOnFilterEvaluationExceptions,
      deadLetteringOnMessageExpiration:
        subscription.settings.deadLetteringOnMessageExpiration,
      defaultRuleOptions: {
        name: '$Default',
        filter: {
          sqlExpression: '1=1',
    ...
    await administrationClient.createSubscription(
      topicId,
      subscription.name,
      body,
    );
  ...
  async updateSubscription(
    ...
    const administrationClient = this.getAdministrationClient();
    const subscriptionProperties = await administrationClient.getSubscription(
      topicId,
      subscription.name,
    );

    subscriptionProperties.autoDeleteOnIdle = !subscription.properties
      .autoDeleteOnIdle
      ? subscriptionProperties.autoDeleteOnIdle
      : subscription.properties.autoDeleteOnIdle;
    subscriptionProperties.defaultMessageTimeToLive =
      subscription.properties.defaultMessageTimeToLive;
    subscriptionProperties.forwardDeadLetteredMessagesTo =
      subscription.properties.forwardDeadLetteredMessagesTo ?? undefined;
    subscriptionProperties.forwardTo =
      subscription.properties.forwardMessagesTo ?? undefined;
    subscriptionProperties.lockDuration = !subscription.properties.lockDuration
      ? subscriptionProperties.lockDuration
      : subscription.properties.lockDuration;
    subscriptionProperties.userMetadata = !subscription.properties.userMetadata
      ? ''
      : subscription.properties.userMetadata;
    subscriptionProperties.enableBatchedOperations =
      subscription.settings.enableBatchedOperations;
    subscriptionProperties.maxDeliveryCount =
      subscription.properties.maxDeliveryCount;
    subscriptionProperties.deadLetteringOnFilterEvaluationExceptions =
      subscription.settings.deadLetteringOnFilterEvaluationExceptions;
    subscriptionProperties.deadLetteringOnMessageExpiration =
      subscription.settings.deadLetteringOnMessageExpiration;
  ...
  async addSubscriptionRule(
    ...
    if (rule.filterType === 'sql') {
      await administrationClient.createRule(
        topicId,
        subscriptionId,
        rule.name,
        {
      ...
      await administrationClient.createRule(
        ...
        rule.name,
        {
          ...
          applicationProperties:
            rule.applicationProperties?.length === 0
              ? undefined
              : applicationProperties,
        },
        {
  ...
  async updateSubscriptionRule(
    ...
    const administrationClient = this.getAdministrationClient();
    if (rule.filterType === 'sql') {
      const ruleToUpdate = await administrationClient.getRule(
        topicId,
        subscriptionId,
        rule.name,
      );
      ...
      ruleToUpdate.action.sqlExpression =
        rule.action === '' ? undefined : rule.action;
      ruleToUpdate.filter = filter;

      await administrationClient.updateRule(
        topicId,
        subscriptionId,
        ruleToUpdate,
      );
    ...
    if (rule.filterType === 'correlation') {
      ...
      filter.correlationId =
        rule.systemProperties?.find((prop) => prop.key === 'correlationId')
          ?.value ?? filter.correlationId;
      filter.messageId =
        rule.systemProperties?.find((prop) => prop.key === 'messageId')
          ?.value ?? filter.messageId;
      filter.to =
        rule.systemProperties?.find((prop) => prop.key === 'to')?.value ??
        filter.to;
      filter.replyTo =
        rule.systemProperties?.find((prop) => prop.key === 'replyTo')?.value ??
        filter.replyTo;
      filter.subject =
        rule.systemProperties?.find((prop) => prop.key === 'subject')?.value ??
        filter.subject;
      filter.sessionId =
        rule.systemProperties?.find((prop) => prop.key === 'sessionId')
          ?.value ?? filter.sessionId;
      filter.replyToSessionId =
        rule.systemProperties?.find((prop) => prop.key === 'replyToSessionId')
          ?.value ?? filter.replyToSessionId;
      filter.contentType =
        rule.systemProperties?.find((prop) => prop.key === 'contentType')
          ?.value ?? filter.contentType;
      filter.applicationProperties =
        rule.applicationProperties?.length === 0
          ? undefined
          : applicationProperties;
  ...
  private async getSubscriptionRules(
  ...
  private mapQueue(
    ...
    queueMeta: QueueRuntimeProperties,
  ): QueueWithMetaData {
    return {
      ...
      properties: {
        ...
        forwardDeadLetteredMessagesTo:
          queue.forwardDeadLetteredMessagesTo ?? null,
  ...
  private mapTopic(
    ...
    topicMeta: TopicRuntimeProperties,
  ): TopicWithMetaData {
    return {
      namespaceId: this.connection.id,
      id: topic.name,
  ...
  private mapSubscription(
    ...
    return {
      ...
      properties: {
        ...
        forwardDeadLetteredMessagesTo:
          subscription.forwardDeadLetteredMessagesTo ?? null,
      ...
      metaData: {
        ...subscriptionMeta,
        endpoint:
          this.getEndpoint() + topicId + '/' + subscription.subscriptionName,
      },
      rules: rules.map((rule) => {
        return (
          'sqlExpression' in rule.filter
            ? {
                filterType: 'sql',
            ...
            : {
                ...
                systemProperties: Object.keys(rule.filter)
                  .map((key) => {
                ...
                applicationProperties: Object.keys(
                  rule.filter.applicationProperties ?? {},
                ).map((key) => {
                  return {
                    key: key,
                    value:
                      (rule.filter as CorrelationRuleFilter)
