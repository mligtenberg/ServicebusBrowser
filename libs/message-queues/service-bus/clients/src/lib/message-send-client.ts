...
export class MessageSendClient {
  constructor(
    ...
    private readonly endpoint: SendEndpoint,
  ) {}

  async send(
    input: contracts.ServiceBusMessage | contracts.ServiceBusMessage[],
  ): Promise<void> {
  ...
  mapMessage(message: contracts.ServiceBusMessage): ServiceBusMessage {
    const duration = message.timeToLive
      ? Duration.fromISO(message.timeToLive)
      : null;
